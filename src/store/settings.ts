  ...
  persist(
    (set, get) => ({
      ...
      updateFieldMapping: (mapping) =>
        set({ fieldMapping: { ...get().fieldMapping, ...mapping } }),

      setPollingInterval: (interval) =>
        set({ pollingInterval: Math.max(10000, interval) }), // Mínimo 10 segundos

      togglePolling: () =>
        set((state) => ({ enablePolling: !state.enablePolling })),

      toggleAnimations: () =>
        set((state) => ({ enableAnimations: !state.enableAnimations })),

      setReducedMotion: (enabled) =>
        set({ showReducedMotion: enabled }),

      setDefaultPageSize: (size) =>
        set({ defaultPageSize: Math.max(10, Math.min(100, size)) }),
...
export const usePollingConfig = () => useSettingsStore((state) => ({
  interval: state.pollingInterval,
  enabled: state.enablePolling,
}))
export const useUIConfig = () => useSettingsStore((state) => ({
  animations: state.enableAnimations,
  reducedMotion: state.showReducedMotion,
}))
export const useTableConfig = () => useSettingsStore((state) => ({
