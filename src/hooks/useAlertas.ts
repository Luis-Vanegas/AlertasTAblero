...
export const useAlertaById = (id: number) => {
  ...
  const mappedAlerta = alerta && typeof alerta === 'object' ? alertasApiService.mapAlerta(alerta) : null
...
export const useAlertasByObra = (obraId: number) => {
  ...
  const mappedAlertas = alertas && Array.isArray(alertas) ? alertasApiService.mapAlertas(alertas) : []
