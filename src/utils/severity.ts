...
import { GRAVEDAD_OPTIONS, IMPACTO_OPTIONS, SEVERITY_THRESHOLDS } from '../constants'

/**
 * Normaliza valores de gravedad a llaves consistentes
 */
export const normalizeGravedad = (value?: string | null): 'crítica' | 'alta' | 'media' | 'leve' | 'baja' | '' => {
  if (!value) return ''
...
export const getImpactMeta = (impacto: string) => {
  ...
  if (v.includes('presupuesto')) return { 
    icon: 'AttachMoney', 
    color: 'secondary', 
    label: 'Presupuesto' 
  }
  if (v.includes('cronograma')) return { 
    icon: 'Schedule', 
    color: 'info', 
    label: 'Cronograma' 
  }
  if (v.includes('alcance')) return { 
    icon: 'TrackChanges', 
    color: 'warning', 
    label: 'Alcance' 
  }
  if (v.includes('comunidad')) return { 
    icon: 'Groups', 
    color: 'success', 
    label: 'Comunidad' 
  }
...
export const calculateSeverity = (alerta: MappedAlerta): 'ok' | 'warning' | 'critical' => {
  ...
  if (SEVERITY_THRESHOLDS.critical.gravedad.includes(gravedad) || gravedad === 'crítica') {
    level = 'critical'
  } else if (SEVERITY_THRESHOLDS.warning.gravedad.includes(gravedad)) {
    level = 'warning'
  } else {
  ...
  if (level !== 'critical') {
    if (SEVERITY_THRESHOLDS.critical.impacto_riesgo.includes(impacto)) {
      level = 'warning'
    } else if (SEVERITY_THRESHOLDS.warning.impacto_riesgo.includes(impacto)) {
 ...
 */
export const isPriorityAlert = (alerta: MappedAlerta, priorityProjects: readonly string[]): boolean => {
  const project = (alerta.proyecto_estrategico || '').toLowerCase().normalize('NFD').replace(/\p{Diacritic}/gu, '')
 ...
 */
export const filterBySeverity = (alertas: MappedAlerta[], severity: 'ok' | 'warning' | 'critical' | 'all'): MappedAlerta[] => {
