...
export class SchemaDetectionService {
  ...
  private matchesFieldPattern(
    ...
    switch (mappedField) {
      ...
      case 'dias_retraso':
        return (
          name.includes('retraso') ||
          name.includes('delay') ||
          name.includes('atraso') ||
          name.includes('dias')
        );
  ...
  private generateSuggestions(fields: FieldMapping[], suggestions: string[]): void {
    ...
    const requiredFields: (keyof Obra)[] = [
      'id',
      'nombre_obra',
      'fecha_inicio',
      'fecha_fin',
    ...
    fields.forEach(field => {
      if (field.confidence < 0.5) {
        suggestions.push(
          `El campo "${field.originalField}" tiene baja confianza de mapeo (${Math.round(field.confidence * 100)}%)`
        );
      }
    });

    // Sugerir campos duplicados
    const fieldCounts = fields.reduce(
      (acc, field) => {
