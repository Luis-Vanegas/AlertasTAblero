...
const env = (import.meta as unknown as { env?: { VITE_API_BASE?: string; VITE_API_KEY?: string } }).env || {}

export class AlertasApiService {
  private baseUrl = env.VITE_API_BASE || 'https://visorestrategicobackend-gkejc4hthnace6b4.eastus2-01.azurewebsites.net/api/powerbi'
  ...
  async getAlertas(filters: ApiFilters = {}): Promise<ApiResponse> {
    try {
      ...
      if (filters.proyecto_estrategico) params.append('proyecto_estrategico', filters.proyecto_estrategico)
      ...
      try {
        const raw = Array.isArray((data as unknown as { data?: unknown[] }).data) ? (data as unknown as { data?: unknown[] }).data! : []
        const distGravedad: Record<string, number> = {}
        const distImpacto: Record<string, number> = {}
        raw.slice(0, 5).forEach((a, idx: number) => {
          console.log(`Ejemplo[${idx}] → GRAVEDAD:`, getField(a, 'GRAVEDAD') ?? getField(a, 'Gravedad') ?? getField(a, 'SEVERIDAD') ?? null, '| IMPACTO:', getField(a, 'IMPACTO RIESGO') ?? null)
        })
        raw.forEach((a) => {
          const g = getString(a, 'GRAVEDAD') ?? getString(a, 'Gravedad') ?? getString(a, 'SEVERIDAD') ?? ''
      ...
      console.log('API Data received:', { count: (data as unknown as { data?: unknown[] }).data?.length, pagination: (data as unknown as { pagination?: unknown }).pagination })
      return data
    } catch (error) {
  ...
  private getMockData(filters: ApiFilters = {}): ApiResponse {
    ...
    if (filters.comuna) {
      filteredData = filteredData.filter(item => 
        getString(item, 'COMUNA O CORREGIMIENTO')?.toLowerCase().includes(filters.comuna!.toLowerCase())
      )
    }

    if (filters.impacto_riesgo) {
      filteredData = filteredData.filter(item => 
        getString(item, 'IMPACTO RIESGO')?.toLowerCase().includes(filters.impacto_riesgo!.toLowerCase())
      )
    }
  ...
  mapAlerta(apiAlerta: ApiAlerta): MappedAlerta {
    const gravedadCampoRaw = getString(apiAlerta, 'GRAVEDAD')
    ...
    const generaCambio = typeof generaCambioRaw === 'string'
    ...
    const fecha_alerta = fechaAlertaRaw && String(fechaAlertaRaw).trim() !== '' ? fechaAlertaRaw : null
  ...
  calculateStats(alertas: MappedAlerta[]): ApiStats {
    ...
    alertas.forEach(alerta => {
      ...
      stats.por_dependencia[alerta.dependencia] = (stats.por_dependencia[alerta.dependencia] || 0) + 1
      
      // Contar por comuna
      stats.por_comuna[alerta.comuna] = (stats.por_comuna[alerta.comuna] || 0) + 1
      
      // Contar por proyecto
      stats.por_proyecto[alerta.proyecto_estrategico] = (stats.por_proyecto[alerta.proyecto_estrategico] || 0) + 1
      
      // Contar por impacto
      stats.por_impacto[alerta.impacto_riesgo] = (stats.por_impacto[alerta.impacto_riesgo] || 0) + 1
  ...
  async getFilterOptions(): Promise<{
    ...
    try {
      ...
      const gravedades = [...new Set(alertas.map(a => a.gravedad).filter((g): g is string => g !== null))].sort()
   ...
   */
  async searchAlertas(searchTerm: string, filters: Omit<ApiFilters, 'search'> = {}): Promise<ApiResponse> {
